...
Periodically, the **Island Management** process culls the lower-performing half of all islands, ranked by their best individual's score. New islands are created to replace them, seeded with elite individuals from the surviving top-performing islands. This process is the sole migration mechanism in the system.
...
- $s_i$: The score of all skeletons in cluster $i$.
- $T_c$: A temperature parameter that anneals according to the schedule $T_c = T_0\left(1 - \frac{u \pmod N}{N}\right)$, where:
    ...
    - $u$: The number of individuals evaluated on the island so far.
    - $N$: The total number of individuals to evaluate before the island is eligible for replacement.
...
The newly evaluated skeleton is added to the cluster corresponding to its score. If no cluster exists for that score, a new one is created.